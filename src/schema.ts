/**
 * PMM Schema — Complete DDL
 * ==========================
 * All CREATE TABLE IF NOT EXISTS statements for the entire PMM platform.
 * Called once at startup by db.ts:ensureDb(). All statements are idempotent.
 *
...
export function initSchema(db: Database): void {
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS portfolio_nodes (
      ...
      type          TEXT NOT NULL CHECK(type IN (
                      ...
                      'workstream','epic','feature','initiative','roadmap'
                    )),
      name          TEXT NOT NULL,
      description   TEXT,
    ...
    CREATE TABLE IF NOT EXISTS harness_adapters (
      id              INTEGER PRIMARY KEY AUTOINCREMENT,
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS roadblocks (
      ...
      blocking_worker_id INTEGER REFERENCES agent_workers(id)
    );
    ...
    CREATE TABLE IF NOT EXISTS alerts (
      ...
      created_at TEXT DEFAULT (datetime('now')),
      resolved_at TEXT,
      doctor_action_id INTEGER REFERENCES doctor_actions(id),
    ...
    CREATE TABLE IF NOT EXISTS patterns (
      ...
      node_id INTEGER REFERENCES portfolio_nodes(id),
      feature_id INTEGER REFERENCES features(id),
      observed_in_session_id INTEGER REFERENCES sessions(id)
    );
    ...
    CREATE TABLE IF NOT EXISTS skills (
      ...
      context TEXT CHECK(context IN ('default','fork')) DEFAULT 'default',
    ...
    CREATE TABLE IF NOT EXISTS tooling (
      ...
      setup_effort TEXT CHECK(setup_effort IN ('low','medium','high')) DEFAULT 'low',
      priority TEXT CHECK(priority IN ('critical','high','medium','low','none')) DEFAULT 'medium',
      docs_url TEXT,
    ...
    CREATE TABLE IF NOT EXISTS verification_runs (
      ...
      node_id INTEGER REFERENCES portfolio_nodes(id),
      feature_id INTEGER REFERENCES features(id),
      ci_run_id INTEGER REFERENCES ci_pipeline_runs(id)
    );
  `);
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS "agent_workers" (
      ...
      harness_name TEXT,
      swarm_task_id INTEGER REFERENCES swarm_tasks(id),
      layer_num INTEGER,
      track_letter TEXT,
    ...
    CREATE TABLE IF NOT EXISTS oracle_insights (
      id            INTEGER PRIMARY KEY AUTOINCREMENT,
      category      TEXT NOT NULL CHECK(category IN ('observation','research','synthesis','proposal')),
      title         TEXT NOT NULL,
      description   TEXT,
      evidence_json TEXT,
      source        TEXT,
      ...
      impact_score  REAL,
      feasibility   REAL,
      status        TEXT DEFAULT 'new' CHECK(status IN ('new','analyzing','proposed','accepted','rejected','implemented')),
    ...
    CREATE TABLE IF NOT EXISTS tool_setup_steps (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
  ...
  // ── Tier 3: Swarm tables referencing agent_workers ──────────────────
  db.exec(`
